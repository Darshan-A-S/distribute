...
export default function TemplateForm({ initial, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-800 rounded-lg p-6 max-w-3xl space-y-4 mb-6">
      ...
      <div>
        ...
        <input
          ...
          className="w-full bg-gray-950 border border-gray-700 rounded px-3 py-2 text-sm focus:outline-none focus:border-indigo-500"
        />
        <p className="text-xs text-gray-600 mt-1">Use {'{variable}'} for dynamic values</p>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          required
          rows={12}
          className="w-full bg-gray-950 border border-gray-700 rounded px-3 py-2 text-sm font-mono focus:outline-none focus:border-indigo-500"
        />
        <p className="text-xs text-gray-600 mt-1">Full HTML supported. Use {'{name}'}, {'{email}'}, etc. for personalization.</p>
      </div>
